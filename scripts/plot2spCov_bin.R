...
if (length(args)==0) {
 stop("\n
  usage:    Rscript --vanilla plot2spCov_bin.R <tag> <path> <winlen>
  example:  Rscript --vanilla plot2spCov_bin.R AllSacch2 CPOPY0_AllSacch2 1kb\n\n", 
  call.=FALSE)
}
...
# weird transform
#full_cov_table$average = full_cov_table$average / 1000
...
for (j in 1:2){
 ...
 axis(side=1, at=int_lenTab$cumLen, labels=FALSE, padj = -0.75)
  ...
  text(x=int_lenTab$midCumLen, y=-2.5, labels=seq(1,nrow(int_lenTab)), srt=-90, xpd=TRUE)
 #}
 #reverse y axis
 #axis(side=2, line=-2, at=seq(-mYlim,mYlim), labels=FALSE, las=2)
 axis(side=2, line=-2, at=c("2","1.5","1","0.5","0","-0.5","-1","-1.5","-2"), labels=FALSE, las=2)
 text(x=-300000, y=c(2,1.5,1,0.5,0,-0.5,-1,-1.5,-2), labels=c("2","1.5","1","0.5","0","-0.5","-1","-1.5","-2"), cex=0.5)

 text(x=-600000, y=-0, labels = strains[j], srt=-90, xpd=TRUE)
 ...
 lines(x=c(0,lngstLen), y=c(1.5,1.5), col = "gray", lty=1, lwd = 0.5)
 lines(x=c(0,lngstLen), y=c(1,1), col = "gray", lty=1, lwd = 0.5)
 lines(x=c(0,lngstLen), y=c(0.5,0.5), col = "gray", lty=1, lwd = 0.5)

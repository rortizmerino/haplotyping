...
#R# def generateLongReadFastQFiles(haplotigReadNameFilePath,longReadFastQFilePath,outputPath):
def generateLongReadFastQFiles(haplotigReadNameFilePath,longReadFastQFilePath,outputPath,phasedData=None):
#R#
    if phasedData!="":
        phasedDataFileName=haplotigReadNameFilePath.replace('clusterReadNames','phasedDataSimple')
        phasedDataFileName=phasedDataFileName.replace('cleaned.','') #R# in case clusters were cleaned
        phasedDataFile=open(phasedDataFileName,"rt")

        haplotigChrDict={}
        haplotigNoDict={}

        for line in phasedDataFile:
            line=line.strip("\n")
            line=line.split()
            haplotigName=line[0]
            haplotigChr=line[3]
            haplotigNo=line[4]
            haplotigChrDict[haplotigName]=haplotigChr
            haplotigNoDict[haplotigName]=haplotigNo
        phasedDataFile.close()
#R#
    ##clusterStatsText=""
    haplotigReadDict={}
...
#R#
        if phasedData!="":
            Chr=str(haplotigChrDict[haplotigName])
            No=str(haplotigNoDict[haplotigName])
            #haplotigName=haplotigName+"_"+Chr+"_"+No
            haplotigName=Chr+"_"+No+"_"+haplotigName
#R#

        #R# haplotigFile=gzip.open(outputPath+haplotigName+".fastq.gz",'wt')
        haplotigFile=open(outputPath+haplotigName+".fasta",'wt')
        haplotigFile.write(haplotigFastQText)
        haplotigFile.close()

    #R#
    #unPhasedFile=gzip.open(outputPath+"unphased.fastq.gz",'wt')
    unPhasedFile=open(outputPath+"unphased.fasta",'wt')
    for readID, readSeq in longReadData.items():
        if readID not in usedLongReadNames:
            #readInfo=readID+"\n"+"\n".join(longReadData[readID])+"\n" #R# fastq format
            readInfo=readID.replace("@",">")+"\n"+longReadData[readID][0]+"\n"
            unPhasedFile.write(readInfo)

...
if (length(args)==0) {
 stop("\n
  usage:    Rscript --vanilla plotAllSacchCov_bin_full.R <tag> <path> <winlen>
  example:  Rscript --vanilla plotAllSacchCov_bin_full.R AllSacch2 CPOPY0_AllSacch2 1kb\n\n", 
  call.=FALSE)
}
...
plot_title = paste(prefix, "_coverage_per", winlen, "_full", ".jpeg", sep="")
plot_mtit  = paste(prefix, "_coverage_per", winlen, "_full", sep="")
...
jpeg(file=plot_title, width=300, height=200, units="mm", quality=80, res=600);
par(mfrow=c(8,1), mar=c(1,2,1.5,1), oma=c(5,3.5,2,1))
...
#for (j in 1:2){
 ...
 axis(side=2, line=-2, at=seq(mYlim,0), labels=FALSE, las=2)
 text(x=-300000, y=seq(mYlim,0), labels=c("0","1","2","3","4"), xpd=TRUE)
 text(x=-600000, y=2, labels = strains[j], xpd=TRUE)
 text(x=int_lenTab$midCumLen, y=4.5, labels=seq(1,nrow(int_lenTab)), xpd=TRUE)

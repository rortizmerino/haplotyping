...
#R#
def printClusters(args):
    print("\nEntering filter option using:")
    print(args.clusterReadNames)
    print(args.longReadFile)

    #Generate FastQ Files
    haplotigReadNameFile=args.clusterReadNames
    fastQFilePrefix=os.path.join(args.outputFolder+args.strainName+"_")

    nPhaseFunctions.generateLongReadFastQFiles(args.clusterReadNames,args.longReadFile,fastQFilePrefix,"1")
...
def main():
    ...
    #R#
    filteringParser=subparsers.add_parser('filter',help='Filter nPhase results.',parents=[parser])
    required_Filtering=filteringParser.add_argument_group('required arguments')
    #R#

    #SHARED REQUIRED ARGUMENTS

    #R# for parserReference in [required_a,required_b,required_c,required_Cleaning]:
    for parserReference in [required_a,required_b,required_c,required_Cleaning,required_Filtering]:
...
#R#
    ...
    required_Filtering.add_argument('--clusteredTSV',required=True,dest='clusterReadNames',help='Path to  clusterReadNames.tsv file, ex: /home/nphaseResults/Individual_1/Phased/Individual_1...clusterReadNames.tsv')
    required_Filtering.add_argument('--output',required=True,dest='outputFolder',help='Path to output folder, ex: /home/phased/')
#R#
    ...
    if "mappedShortReads" in dir(args) or "vcf" in dir(args) or "mappedLongReads" in dir(args):
        ...
        cleaning(args)
#R#
    elif "clusterReadNames" in dir(args):
        printClusters(args)
#R#
    else:
